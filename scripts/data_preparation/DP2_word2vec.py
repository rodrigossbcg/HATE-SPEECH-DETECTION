...
def word2vec(train_df, test_df, vector_size, text_column='text'):
    """
    Train a Word2Vec model on the training set if not already trained, encode both training and testing sets,
    ...
    model_path (str): Path to save or load the Word2Vec model.
    ...
    model_path = f"data/models/word2vec/word2vec_{vector_size}.model"
    ...
    if os.path.exists(model_path):
        ...
        print(f"Loaded existing Word2Vec model with {vector_size} dimensions.")
    else:
    ...
    train_vectors = generate_word_vectors(train_sentences, word2vec_model)
    test_vectors = generate_word_vectors(test_sentences, word2vec_model)
    ...
    train_df_transformed.to_csv(
        f'data/vectors/word2vec/train_{vector_size}.csv', index=False)
    test_df_transformed.to_csv(
        f'data/vectors/word2vec/test_{vector_size}.csv', index=False)
